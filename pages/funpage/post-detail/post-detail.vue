<template>
	<view>
		...
		<view style="display: flex;align-items: center; margin-top: 30rpx;">
			<view style="width: 16rpx; height: 16rpx; border-radius: 50%; background-color: yellow; margin-left: 5rpx;"></view>
			<view style="margin-left: 15rpx; ">内容</view>
		</view>
		<view class="post-detail-info-box">
			<view class="post-content">{{postData.content}}</view>
			<view class="image-box">
				<image v-for="(item,index) in imageUrlList" :key="index" :src="imageBaseUrl+item"  @click="showImage(index)"></image>
			</view>
		</view>
		<view class="contact-info">
			...
			<view v-if="isShowContactMethod" class="contact">
				<view @click="copyInfo(postData.contactor)">联系人: <text>{{postData.contactor}}</text><text v-if="postData.contactor" style="color: red;">【点击复制】</text></view>
				<view @click="copyInfo(postData.phone)">电话: <text>{{postData.phone}}</text><text v-if="postData.phone" style="color: red;">【点击复制】</text></view>
				<view @click="copyInfo(postData.wechat)">微信: <text>{{postData.wechat}}</text><text v-if="postData.wechat" style="color: red;">【点击复制】</text></view>
				<view @click="copyInfo(postData.qq)">QQ: <text>{{postData.qq}}</text><text v-if="postData.qq" style="color: red;">【点击复制】</text></view>
			</view>
		</view>
		<view style="display: flex;align-items: center; margin-top: 30rpx;">
			<view style="width: 16rpx; height: 16rpx; border-radius: 50%; background-color: #333; margin-left: 5rpx;"></view>
			...
			<view class="comment-item" v-for="(comment,index) in commentList" :key="index">
				...
				<view class="other-info-box">
					<view class="delet" v-if="comment.openid==userInfo.openid" style="margin:0rpx 15rpx;">
						<!-- 删除评论 -->
						<uni-icons type="trash" size="22" color="red" @click="deletComment(comment,index,null)"></uni-icons>
					</view>
					<view class="reply" @click="gotoReport('commentOfParent',index)">
						<image src="../../../static/举报.png"></image>
					</view>
					<view class="like">
						<image v-if="currentUserLikePostFlag[index]==false" src="../../../static/赞.png" @click="clickLike(index)"></image>
					...
					<view class="comment-box" v-for="(commentChild,childIndex) in commentListOfChild" :key="childIndex">
						<view class="comment-item" style="border-bottom: 0rpx;">
							<view class="base-info-box" @click="replyComment(comment,commentChild)">
								<view class="avatar" v-cloak>
									<image :src="baseAvatarUrl + commentChild.avatar +'?time='+new Date().getTime()"></image>
								</view>
								<view class="name-and-time">
									<view v-if="commentChild.openid != userInfo.openid">{{commentChild.name}} > {{commentChild.otherName}}</view>
									<view v-else><text style="color: #1296db;"> 我 &nbsp;</text> > {{commentChild.otherName}}</view>
							...
							<view class="other-info-box">
								<view class="delet" v-if="commentChild.openid==userInfo.openid">
									<!-- 删除评论 -->
									<uni-icons type="trash" size="22" color="red" @click="deletComment(commentChild,childIndex,comment)"></uni-icons>
								</view>
								<view class="reply" @click="gotoReport('commentOfChild',childIndex)">
									<image src="../../../static/举报.png"></image>
								</view>
								<view class="like">
									<image v-if="currentUserLikePostFlagOfChild[childIndex].like==false" src="../../../static/赞.png" @click="clickLikeOfChild(index,childIndex)"></image>
									<image v-else src="../../../static/赞 (1).png" @click="clickLikeOfChild(index,childIndex)"></image>
		...
		<view class="comment-input">
			<view v-if="replayTipText!=''" style="color: lightgray;margin-left: 15rpx;display:flex;align-items:center;">
				...
				<uni-icons type="trash-filled" size="20" style="margin-left: 30rpx;" @click="resetPostCommentForm"></uni-icons>

			</view>
			<view ref="commentInput" style="display: flex; justify-content: space-between; align-items: center;">
				<view style="margin-left: 30rpx; width: 80%;"><uni-easyinput v-model="postCommentInfo.content" borderColor="#fff"  style="border: #1296db; margin-left: 15rpx;" placeholder="请输入评论" :focus="fcous"></uni-easyinput></view>
				<uni-icons type="paperplane-filled" size="30" style="color: #2f4052;margin:0 20rpx;" @click="postComment()"></uni-icons>
			</view>
		</view>
	</view>
</template>
